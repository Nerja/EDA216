...
public class BookingPane extends BasicPane {
    ...
    private void fillNameList() {
        ...
        for(String movieName : db.getMovies())
        	nameListModel.addElement(movieName);
    }
    ...
    class DateSelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            if(perf != null) {
                fields[MOVIE_NAME].setText(perf.movieName);
                fields[PERF_DATE].setText(perf.date);
                fields[THEATER_NAME].setText(perf.theaterName);
                fields[FREE_SEATS].setText(perf.freeSeats + "");
            }
        }
    }
    ...
    class ActionHandler implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String movieName = nameList.getSelectedValue();
            String date = dateList.getSelectedValue();
            if(Integer.parseInt(fields[FREE_SEATS].getText()) > 0) {
            	int nbr = db.makeReservation(movieName, date);
            	if(nbr >= 0) {
            		messageLabel.setText("One ticket booked! Ticket nbr: " + nbr);
            	} else {
            		messageLabel.setText("Must be > 0 free seats");
            	}
            } else {
            	messageLabel.setText("Must be > 0 free seats");
            }
